...
Else 
	...
	End If
	Title = "Recherche les musiques dans "& DblQuote(Folder) & " GoogleHome Control"
	WaitingMsg = "Patienter ... Recherche les musiques dans : <font color=Yellow>"& DblQuote(Folder) & "</font> en cours ..."
	...
	Dim Sound,Copyright
	Copyright = " GoogleHome by ABOAT"
	If FSO.FileExists(MyPlayList) = true then 
	ws.Run (MyPlayList)
	Else
	Ws.popup Err.Description & VbCrlF &_
		"Erreur media non trouver !","3",Err.Description & Copyright,VbCritical
		wscript.quit()
	End if 
End If
'*********************************************************
...
Sub CreateProgressBar(Title,WaitingMsg)
	Dim f,f2,ts,ts2,Ligne,i,fread,LireTout,NbLigneTotal,Temp,PathOutPutHTML,fhta,oExec
	Temp = WS.ExpandEnvironmentStrings("%Temp%")
